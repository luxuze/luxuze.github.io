...
// 45/45 cases passed (144 ms)
// Your runtime beats 6.87 % of golang submissions
// Your memory usage beats 38.46 % of golang submissions (7.3 MB)
import (
	"sort"
)
...
func findMinArrowShots(points [][]int) int {
	...
	for i := 0; i < len(points); i++ {
		if points[i][0] <= cursor && points[i][1] >= cursor {
			continue
		}
		result++
		cursor = points[i][1]
